...
def _smoke_cases(include_finalize: bool = True) -> List[SmokeCase]:
    ...
    cases = [
        ...
        SmokeCase(
            ...
            required_stdout='"schema_version": "impact-receipt-compare/v1"',
        ),
        SmokeCase(
            "impact-receipt-compare markdown",
            [
                "impact-receipt-compare",
                "--base",
                "{root}/examples/impact_artifact_receipt.json",
                "--current",
                "{root}/examples/impact_artifact_receipt.json",
                "--format",
                "markdown",
            ],
            required_stdout="# Market Catalyst Impact Receipt Compare",
        ),
