...
- Use `impact-capture-checklist` when the user needs a public-safe screenshot/GIF capture checklist for impact examples, including exact render commands, source fixtures, output paths, existing hashes/bytes, capture targets, and no-live-data/no-broker/no-prediction/no-advice/no-private-data boundaries.
- Use `visual-evidence-receipt` when the user needs release evidence for checked-in static dashboard/report visuals, including paths, hashes, roles, routes, regeneration commands, capture commands, and static-only/local-fixtures/no-live-data/no-broker/no-orders/no-advice/no-private-data boundaries.
