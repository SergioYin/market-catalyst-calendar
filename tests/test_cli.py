...
from market_catalyst_calendar.impact_brief import impact_brief_json
...
class CliTests(unittest.TestCase):
    ...
    def test_taxonomy_json_reports_domain_rules_diagnostics_and_commands(self):
        ...
        self.assertIn("impact-artifact-receipt", {command["id"] for command in payload["commands"]})
        self.assertIn("impact-capture-checklist", {command["id"] for command in payload["commands"]})
    ...
    def test_impact_compare_json_reports_dataset_attention_evidence_and_flag_deltas(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp) / "base.json"
            current = Path(tmp) / "current.json"
            base.write_text(json.dumps(DEMO_DATA), encoding="utf-8")
            ...
            result = self.run_cli("impact-compare", "--base", str(base), "--current", str(current), "--as-of", "2026-05-27")

        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["schema_version"], "impact-compare/v1")
        self.assertEqual(payload["base_input_type"], "dataset")
        self.assertEqual(payload["current_input_type"], "dataset")
        ...
        self.assertEqual(payload["summary"]["attention_score_movement_count"], 1)
        self.assertEqual(payload["summary"]["evidence_state_movement_count"], 1)
        self.assertEqual(payload["summary"]["impact_flag_change_count"], 1)
        self.assertEqual(payload["added"][0]["id"], "demo-amzn-kuiper-2026")
        changed = payload["changed"][0]
        self.assertEqual(changed["id"], "demo-nvda-computex-2026")
        self.assertEqual(changed["attention_score_change"], {"delta": -3, "from": 96, "to": 93})
        self.assertEqual(changed["evidence_state_transition"], {"from": "stale", "to": "fresh"})
        self.assertEqual(changed["impact_flag_change"]["removed_flags"], ["stale_review"])

    def test_impact_compare_accepts_generated_impact_brief_snapshots(self):
        base_dataset = parse_dataset(DEMO_DATA)
        current_dataset = parse_dataset(DEMO_UPDATED_DATA)
        base_brief = impact_brief_json(base_dataset.records, base_dataset.as_of, 45, 14)
        current_brief = impact_brief_json(current_dataset.records, current_dataset.as_of, 45, 14)
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp) / "base_brief.json"
            current = Path(tmp) / "current_brief.json"
            base.write_text(json.dumps(base_brief), encoding="utf-8")
            current.write_text(json.dumps(current_brief), encoding="utf-8")

            result = self.run_cli("impact-compare", "--base", str(base), "--current", str(current), "--format", "markdown")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("# Market Catalyst Impact Compare", result.stdout)
        self.assertIn("Base snapshot: 2026-05-13 (impact-brief)", result.stdout)
        self.assertIn("## Changed Catalysts", result.stdout)
        self.assertIn("demo-nvda-computex-2026", result.stdout)
    ...
    def test_demo_bundle_writes_every_example_with_manifest_and_transcript(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("examples/impact_compare.json", paths)
    ...
    def test_impact_artifact_receipt_json_captures_paths_hashes_schema_and_boundaries(self):
        result = self.run_cli("impact-artifact-receipt", "--root", str(ROOT))

        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["schema_version"], "impact-artifact-receipt/v1")
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["summary"]["artifact_count"], 6)
        self.assertEqual(payload["missing"], [])
        self.assertIn("No live data", payload["boundaries"][0])
        self.assertIn("No broker integration", payload["boundaries"][1])
        self.assertIn("No prediction", payload["boundaries"][2])
        self.assertIn("No investment advice", payload["boundaries"][3])

        artifacts = {item["name"]: item for item in payload["artifacts"]}
        brief = artifacts["impact_brief.json"]
        expected_text = (ROOT / "examples" / "impact_brief.json").read_text(encoding="utf-8")
        self.assertEqual(brief["schema_label"], "impact-brief/v1")
        self.assertEqual(brief["output_path"], "examples/impact_brief.json")
        self.assertEqual(brief["bytes"], len(expected_text.encode("utf-8")))
        self.assertEqual(len(brief["sha256"]), 64)
        self.assertEqual(brief["inputs"][0]["path"], "examples/demo_records.json")
        self.assertIn("> examples/impact_brief.json", brief["rerun_command"])

        compare = artifacts["impact_compare.md"]
        self.assertEqual(compare["schema_label"], "impact-compare/markdown")
        self.assertEqual([item["path"] for item in compare["inputs"]], ["examples/demo_records.json", "examples/demo_records_updated.json"])

    def test_impact_artifact_receipt_markdown_renders_release_table(self):
        result = self.run_cli("impact-artifact-receipt", "--root", str(ROOT), "--format", "markdown")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("# Market Catalyst Impact Artifact Receipt", result.stdout)
        self.assertIn("No live data", result.stdout)
        self.assertIn("| `impact_dashboard.json` | `impact-dashboard/v1` |", result.stdout)
        self.assertIn("`python -m market_catalyst_calendar impact-compare", result.stdout)

    def test_impact_capture_checklist_json_captures_public_safe_render_targets(self):
        result = self.run_cli("impact-capture-checklist", "--root", str(ROOT))

        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["schema_version"], "impact-capture-checklist/v1")
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["summary"]["item_count"], 4)
        self.assertEqual(payload["missing"], [])
        self.assertFalse(payload["boundary"]["live_data"])
        self.assertFalse(payload["boundary"]["broker_connectivity"])
        self.assertFalse(payload["boundary"]["prediction"])
        self.assertFalse(payload["boundary"]["investment_advice"])
        self.assertFalse(payload["boundary"]["private_data"])
        self.assertFalse(payload["boundary"]["trade_recommendation"])
        self.assertIn("No live data", payload["boundaries"][0])
        self.assertIn("No broker integration", payload["boundaries"][1])
        self.assertIn("No predictions", payload["boundaries"][2])
        self.assertIn("No investment advice", payload["boundaries"][3])
        self.assertIn("No private data", payload["boundaries"][4])

        self.assertEqual(
            [item["artifact"] for item in payload["checklist"]],
            ["impact-brief", "impact-dashboard", "impact-compare", "impact-artifact-receipt"],
        )
        items = {item["artifact"]: item for item in payload["checklist"]}
        brief = items["impact-brief"]
        expected_text = (ROOT / "examples" / "impact_brief.md").read_text(encoding="utf-8")
        self.assertEqual(brief["source_fixture"], "examples/demo_records.json")
        self.assertEqual(brief["output_artifact_path"], "examples/impact_brief.md")
        self.assertEqual(brief["existing"]["bytes"], len(expected_text.encode("utf-8")))
        self.assertEqual(
            brief["existing"]["sha256"],
            "90188731ad7ded160244fb18a7dca8dded8c119df87a8455532ad8f1bf3618f6",
        )
        self.assertIn("> examples/impact_brief.md", brief["render_command"])
        self.assertIn("screenshot or GIF", brief["capture_target"])

        compare = items["impact-compare"]
        self.assertEqual(compare["source_fixture"], "examples/demo_records.json; examples/demo_records_updated.json")
        self.assertIn("--format markdown", compare["render_command"])
        self.assertEqual(compare["output_artifact_path"], "examples/impact_compare.md")

    def test_impact_capture_checklist_markdown_renders_checkboxes(self):
        result = self.run_cli("impact-capture-checklist", "--root", str(ROOT), "--format", "markdown")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("# Market Catalyst Impact Capture Checklist", result.stdout)
        self.assertIn("No live data", result.stdout)
